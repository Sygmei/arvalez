    ...
    match cli.command {
        ...
        Command::Generate {
            ...
            if typescript_enabled {
                let effective_template_dir = config_file.output.typescript.template_dir.clone();
                let (common, ts_config) = resolve_typescript_config(
                    &config_file,
                    None,
                    false,
                    output_version.clone(),
                ...
                let files = timing_collector.measure_result("typescript_generate", || {
                    generate_typescript(&ir, effective_template_dir.as_deref(), &common, &ts_config)
                })?;
        ...
        Command::GenerateTypescript {
            ...
            let effective_template_dir = template_dir
                .or(config_file.output.typescript.template_dir.clone());
            let (common, ts_config) = resolve_typescript_config(
                &config_file,
                package_name,
                group_by_tag,
